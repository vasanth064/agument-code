...
const userProfilePath = 'public/images/users';
const userMulterStorage = new diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(
      null,
      `user-${Math.random().toString(36).substring(2, 25)}-${Date.now()}.${ext}`
    );
  },
});

const userMulterFilter = (req, file, cb) => {
...
export const getUsers = async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('No users found add a user !', 404));
  }
  res.status(200).json({
    status: 'success',
    results: user.length,
...
export const updateCurrentUser = catchAsync(async (req, res, next) => {
  if (req.body.password || req.body.passwordConfirm) {
    return next(
      new AppError('Use password update for updating the password !', 400)
    );
  }
  const validatedBody = filterObject(req.body, ['name', 'email']);
  if (req.file) validatedBody.photo = `${userProfilePath}/${req.file.filename}`;

  const updatedUser = await User.findByIdAndUpdate(req.user.id, validatedBody, {
    new: true,
    runValidators: true,
  });
